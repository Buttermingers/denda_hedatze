...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$descuento_options = ['options' => ['min_range' => 0, 'max_range' => 100, 'default' => 0]];
	...
	if (!empty($izena) && !empty($deskribapena) && $prezioa !== false && $kategoria_id !== false && $descuento !== false) {
		...
		if ($produktuaDB->gehitu($produktua)) {
			include 'produktu_gorde_da.php';
			exit;
		}
	}

	$mezua = "Errorea: Datu batzuk ez dira zuzenak. Ziurtatu derrigorrezko eremuak (izena, deskribapena, prezioa, kategoria) ondo beteta daudela eta deskontua 0 eta 100 artean dagoela.";
}
